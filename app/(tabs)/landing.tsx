...
export default function TucanLanding() {
  ...
  // Definir roles dentro del componente para tener acceso a t()
  const roles: RoleDef[] = [
    { key: 'pasajero', label: t('landing.roles.pasajero'), desc: t('landing.descriptions.pasajero'), iconBg: '#e3f9ec', icon: '👤', tipo: 1, target: '/passenger' },
    { key: 'chofer', label: t('landing.roles.chofer'), desc: t('landing.descriptions.chofer'), iconBg: '#e3f9ec', icon: '🚌', tipo: 2, target: '/chofer' },
    { key: 'dueno_micro', label: t('landing.roles.dueno_micro'), desc: t('landing.descriptions.dueno_micro'), iconBg: '#e3f9ec', icon: '🚐', tipo: 3, target: '/dueno' },
    { key: 'dueno_sindicato', label: t('landing.roles.dueno_sindicato'), desc: t('landing.descriptions.dueno_sindicato'), iconBg: '#e3f9ec', icon: '📊', tipo: 4, target: '/sindicato' },
  ];
  ...
  return (
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={{ paddingBottom: 200 }}>
        <View style={styles.headerWrapper}>
          ...
          <View style={styles.heroContent}>
            <ThemedText type="title" style={styles.heroTitle}>{t('landing.heroTitle')}</ThemedText>
            <ThemedText style={styles.heroSubtitle}>{t('landing.heroSubtitle')}</ThemedText>
          </View>
        </View>
      ...
      <View style={styles.bottomBar}>
        <NavItem label={t('landing.navigation.home')} active={activeTab === 'home'} onPress={() => setActiveTab('home')} />
        <NavItem label={t('landing.navigation.more')} active={activeTab === 'more'} onPress={() => setActiveTab('more')} />
      </View>
    </View>
  );
}
