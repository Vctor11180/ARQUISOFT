...
export default function PassengerPanel() {
	...
	const handleVincularCuenta = async () => {
		...
		setTimeout(() => {
			if (cards.length > 0) {
				cards[0].saldo += 50;
				Alert.alert(t('passenger.alerts.rechargeSuccess'), t('passenger.alerts.rechargeMessage'));
			} else {
				Alert.alert(t('passenger.alerts.noCards'), t('passenger.alerts.noCardsMessage'));
			}
			setRecargando(false);
		}, 1200);
	};
		...
		<ScrollView style={styles.root} contentContainerStyle={{ paddingBottom: 60 }}>
			...
			<LinearGradient colors={[BRAND_DARK, BRAND_PRIMARY]} style={styles.headerGradient}>
				<View style={styles.headerTop}>
					<ThemedText style={styles.greeting}>{t('passenger.greeting', { userName })}</ThemedText>
				...
				<View style={styles.balanceCard}>
					<View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
						...
						<ThemedText style={styles.balanceLabel}>{t('passenger.balance')}</ThemedText>
					</View>
				...
				<View style={styles.quickRow}>
					<QuickOption icon="creditcard" label={t('passenger.cards')} />
					<QuickOption icon="watchface" label={t('passenger.wearables')} />
					<QuickOption icon="wave.3.right" label={recargando ? t('passenger.connecting') : t('passenger.link')} onPress={handleVincularCuenta} active={recargando} />
				</View>
				...
				<Pressable onPress={() => router.push('/interfazPago')} style={({ pressed }) => [styles.payNfcBtn, pressed && { transform: [{ scale: 0.97 }] }]}>
					...
					<ThemedText style={styles.payNfcText}>{t('passenger.payNFC')}</ThemedText>
				...
				{showNFC && (
					<View style={styles.nfcInline}>
						...
						<View style={{ flex: 1 }}>
							<ThemedText style={styles.nfcHint}>{t('passenger.nfcHint')}</ThemedText>
							<ThemedText style={styles.lastPay}>{t('passenger.nfcReady')}</ThemedText>
						</View>
						<Pressable style={styles.nfcBtnSmall}><ThemedText style={styles.nfcBtnTxt}>{t('passenger.pay')}</ThemedText></Pressable>
					</View>
				)}
			</LinearGradient>
			...
			<View style={styles.statsRow}>
				<StatPill icon="creditcard" label={t('passenger.cards')} count={cards.length} total={5} color={BRAND_PRIMARY} />
				<StatPill icon="watchface" label={t('passenger.wearables')} count={wearables.length} total={5} color={BRAND_PRIMARY} />
			</View>

			{/* Tarjetas */}
			<SectionHeader title={t('passenger.cardsTitle')} actionLabel={t('passenger.add')} onAction={async () => {
				if (cards.length >= 5) { Alert.alert(t('passenger.alerts.limitReached'), t('passenger.alerts.maxCards')); return; }
				const { error } = await createCard(t('passenger.newCard'));
				if (error) Alert.alert(t('passenger.alerts.error'), error.message || t('passenger.alerts.createError'));
			...
			<View style={styles.cardList}>
				{cards.map(c => <PaymentCardItem key={c.id} card={c as any} onToggle={() => toggleActive(c.id)} onDelete={() => {
					Alert.alert(t('passenger.actions.deleteCard'), t('passenger.actions.deleteConfirm'), [
						{ text: t('passenger.actions.cancel'), style: 'cancel' },
						{
							text: t('passenger.actions.delete'), style: 'destructive', onPress: async () => {
								const { error } = await deleteCard(c.id);
								if (error) Alert.alert(t('passenger.alerts.error'), error.message || t('passenger.alerts.deleteError'));
							}
						}
					]);
				}} />)}
			...
			{/* Manillas */}
			<SectionHeader title={t('passenger.wearablesTitle')} actionLabel={t('passenger.add')} onAction={() => {
				if (wearables.length >= 5) return;
				const id = 'w' + (wearables.length + 1);
				setWearables(prev => [...prev, { id, alias: t('passenger.newWearable') + ' ' + id, activa: true, bateria: 100, viajesMes: 0 }]);
			...
			<View style={styles.wearableList}>
				{wearables.map(w => <WearableItem key={w.id} item={w} onToggle={() => setWearables(prev => prev.map(p => p.id === w.id ? { ...p, activa: !p.activa } : p))} onDelete={() => {
					Alert.alert(t('passenger.actions.deleteWearable'), t('passenger.actions.deleteConfirm'), [
						{ text: t('passenger.actions.cancel'), style: 'cancel' },
						{ text: t('passenger.actions.delete'), style: 'destructive', onPress: () => setWearables(prev => prev.filter(p => p.id !== w.id)) }
					]);
				}} />)}
			</View>
		</ScrollView>
		...
		<Pressable onLongPress={onToggle} style={({ pressed }) => [styles.payCard, pressed && { transform: [{ scale: 0.97 }] }]}>
			<LinearGradient colors={[card.colorA || '#42af56', card.colorB || '#2e8741']} style={styles.payCardGradient}>
				<View style={styles.payCardTopRow}>
					...
					<View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
						<Pressable onPress={onToggle} style={[styles.statusBadge, card.activa ? styles.badgeActiva : styles.badgeInactiva]}>
							<ThemedText style={styles.badgeText}>{card.activa ? t('passenger.status.active') : t('passenger.status.inactive')}</ThemedText>
						</Pressable>
				...
				<View style={styles.payFooterRow}>
					<ThemedText style={styles.paySaldo}>{t('passenger.credit')} {card.saldo.toFixed(2)}</ThemedText>
					<ThemedText style={styles.payTipo}>{card.tipo === 'FISICA' ? t('passenger.type.physical') : t('passenger.type.virtual')}</ThemedText>
				</View>
				<View style={styles.payStatsRow}>
					<ThemedText style={styles.payViajes}>{card.viajesMes} {t('passenger.stats.tripsThisMonth')}</ThemedText>
					{card.ultimoAbordaje && <ThemedText style={styles.payUltUso}>{card.ultimoAbordaje}</ThemedText>}
				</View>
				{card.rutaUltima && <ThemedText style={styles.payRuta}>{t('passenger.stats.lastRoute')} {card.rutaUltima}</ThemedText>}
			</LinearGradient>
		</Pressable>
	);
}
	...
	return (
		<Pressable onLongPress={onToggle} style={({ pressed }) => [styles.wearableCard, pressed && { transform: [{ scale: 0.97 }] }]}>
			...
			<View style={styles.wearRow}>
				<ThemedText style={styles.wearBattery}>{item.bateria} {t('passenger.stats.battery')}</ThemedText>
				<ThemedText style={styles.wearEstado}>{item.activa ? t('passenger.status.active') : t('passenger.status.inactiveWearable')}</ThemedText>
			</View>
			<View style={styles.wearRow}>
				<ThemedText style={styles.wearViajes}>{item.viajesMes} {t('passenger.stats.tripsThisMonth')}</ThemedText>
				{item.ultimoAbordaje && <ThemedText style={styles.wearUltUso}>{item.ultimoAbordaje}</ThemedText>}
			</View>
			{item.rutaUltima && <ThemedText style={styles.wearRuta}>{t('passenger.stats.route')} {item.rutaUltima}</ThemedText>}
			{item.ultima && <ThemedText style={styles.wearUltima}>{t('passenger.stats.lastSync')} {item.ultima}</ThemedText>}
			<ThemedText style={styles.wearHint}>{t('passenger.actions.longPressTo')} {item.activa ? t('passenger.actions.block') : t('passenger.actions.activate')}</ThemedText>
		</Pressable>
	);
}
