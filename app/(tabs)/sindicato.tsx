	...
	return (
		<ScrollView style={styles.screen} contentContainerStyle={{ paddingBottom:50 }}>
			<LinearGradient colors={['#064420', PRIMARY]} style={styles.hero}>
				<ThemedText style={styles.heroTitle}>Panel Sindicatos</ThemedText>
				<ThemedText style={styles.heroSubtitle}>Visión consolidada</ThemedText>
			</LinearGradient>

			<View style={styles.totalsRow}>
				<TotalBox label='Sindicatos' value={sindicatos.length.toString()} />
				<TotalBox label='Micros' value={totalMicros.toString()} />
				<TotalBox label='Choferes' value={totalChoferes.toString()} />
				<TotalBox label='Activos' value={totalActivos.toString()} />
			</View>

			<View style={styles.ingresosBox}>
				<ThemedText style={styles.ingresosLabel}>Ingresos hoy</ThemedText>
				<ThemedText style={styles.ingresosValue}>{totalIngresos} Bs</ThemedText>
			</View>

			<ThemedText style={styles.sectionTitle}>Sindicatos que gestiono</ThemedText>
			{sindicatos.map(s => (
				<Pressable key={s.id} style={({pressed})=>[styles.sindicatoCard, pressed && styles.sindicatoCardPressed]}>          
					...
					<View style={styles.sBottomRow}>
						<View style={styles.varWrapper}>
							...
							<ThemedText style={styles.varLabel}>Var. hoy</ThemedText>
						</View>
			...
			<Pressable style={styles.addButton} onPress={()=>{/* futuro: agregar sindicato */}}>
				<ThemedText style={styles.addButtonTxt}>Agregar sindicato</ThemedText>
			</Pressable>
		</ScrollView>
	);
}
